...
def gerar_acesso_medico(request):
    if request.method == "GET":
        # O HTMLprecisa de dados do acesso medico existentes no banco para preencher uma tabela ao carregar a pagina
        # entao criamos um objeto com todos os dados do banco AcessoMedico com o usuario LOGADO como filtro
        acessos_medico = AcessoMedico.objects.filter(usuario=request.user)
        # passa a instancia do SolicitarExame - exame como context da URL
        # assim é possivel referenciar a tabela e tabelas relacionadas no HTML
        # enviamos esse objeto para o HTML durante a operacao de RENDER com o alias acessos_medico
        return render(request, 'gerar_acesso_medico.html', {'acessos_medico':acessos_medico})
    elif request.method == "POST":
        # recebe do HTML TODOS os dados do formulario gerar acesso medico
        ...
        # apos instanciar e carregar as variaveis, salva no banco de dados:
        acesso_medico.save()

        messages.add_message(request, constants.SUCCESS, 'Acesso cadastrado com sucesso.')
        return redirect('/exames/gerar_acesso_medico/')

# o medico só quer acessar o exame através do TOKEN, sem fazer login
def acesso_medico(request, token):
    ...
    # colocando dois filtros seria como um e.
